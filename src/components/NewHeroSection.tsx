...
import { Cog, ChevronDown, Settings, Wrench } from 'lucide-react';
...
const NewHeroSection = () => {
  ...
  return (
    <SteampunkBackground variant="rich" className="vintage-paper-light weathered-edges min-h-screen flex items-center justify-center pt-8 sm:pt-16 md:pt-0">
      <section 
        className="relative overflow-hidden w-full"
        itemScope 
        itemType="https://schema.org/Organization"
      >
        {/* Enhanced Animated Clockwork Background */}
        <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
          {/* Main gears */}
          <div className="absolute top-20 left-10 w-32 h-32 opacity-15">
            <Cog className="w-full h-full text-brass gear-rotation-slow" />
          </div>
          <div className="absolute top-40 right-20 w-24 h-24 opacity-20">
            <Settings className="w-full h-full text-oxidized-teal gear-rotation-fast" />
          </div>
          <div className="absolute bottom-32 left-1/4 w-20 h-20 opacity-12">
            <Wrench className="w-full h-full text-brass mechanical-tick" />
          </div>
          <div className="absolute bottom-20 right-1/3 w-16 h-16 opacity-18">
            <Cog className="w-full h-full text-oxidized-teal gear-wobble" />
          </div>
          
          {/* Additional mechanical elements */}
          <div className="absolute top-1/2 left-8 w-12 h-12 opacity-10">
            <Settings className="w-full h-full text-brass clockwork-tick" />
          </div>
          <div className="absolute top-1/3 right-8 w-14 h-14 opacity-15">
            <Wrench className="w-full h-full text-oxidized-teal piston-move" />
          </div>
          
          {/* Steam effects */}
          <SteamEffect intensity="medium" className="top-16 left-16" />
          <SteamEffect intensity="low" className="top-32 right-32" />
          <SteamEffect intensity="high" className="bottom-40 left-20" />
        </div>

        <div className="container mx-auto px-6 relative z-10 text-center">
          {/* Company Logo and Title */}
          <header className="animate-fade-in">
            {/* 3D Company Logo with enhanced animations */}
            <div className="flex justify-center mb-1 sm:mb-6 relative">
              <div className="relative">
                <img 
                  ...
                  className="w-56 h-56 sm:w-64 sm:h-64 md:w-80 md:h-80 object-contain gear-rotation-slow hover:gear-wobble transition-all duration-300"
                  width="320"
                  height="320"
                  loading="eager"
                  itemProp="logo"
                />
                {/* Steam effects around logo */}
                <SteamEffect intensity="low" className="top-8 left-8" />
                <SteamEffect intensity="low" className="top-8 right-8" />
                <SteamEffect intensity="medium" className="bottom-16 left-1/2 transform -translate-x-1/2" />
              </div>
            </div>
